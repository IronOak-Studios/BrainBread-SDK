...
int CHudSpectator::Init()
{
	...
	m_autoDirector	= gEngfuncs.pfnRegisterVariable("spec_autodirector","0",0);	// + BB: off by default
	m_pip			= gEngfuncs.pfnRegisterVariable("spec_pip","0",0);			// + BB: off by default
...
void CHudSpectator::SetModes(int iNewMainMode, int iNewInsetMode)
{
	...
	if ( iNewMainMode != g_iUser1 )
	{
		...
		if ( (g_iUser1 == OBS_IN_EYE) || (g_iUser1 == OBS_ROAMING) ) 
		{
			...
			//SetCrosshair( m_hCrosshair, m_crosshairRect, 255, 255, 255 );	// + BB: disabled, mod has own crosshair
		}
		else
		{
			memset( &m_crosshairRect,0,sizeof(m_crosshairRect) );
			//SetCrosshair( 0, m_crosshairRect, 0, 0, 0 );					// + BB: disabled
		} 
...
void CHudSpectator::InitHUDData()
{
	...
	SetModes( OBS_CHASE_FREE, INSET_OFF );	// + BB: free chase by default
