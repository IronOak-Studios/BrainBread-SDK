...
//		char tempString[128];
		...
		*/}
		...
		if ( ( (g_iUser1 == OBS_IN_EYE) || (g_iUser1 == OBS_CHASE_FREE) || (g_iUser1 == OBS_CHASE_LOCKED) ) && player )
		{
		...
		*/
			_snprintf( szText, 63, "Not yet undead %d : %d Zombies", g_TeamInfo[1].frags, g_TeamInfo[2].frags );
		//}
		...
		//Spin
		int timer = (int)( gHUD.m_Counter.m_flRoundTime );
...
void TeamFortressViewport::ShowVGUIMenu( int iMenu )
{
	...
	if ( gHUD.m_iIntermission && iMenu != MENU_INTRO && iMenu != MENU_TEAM )
		return;
...
// + Spin
CMenuPanel* TeamFortressViewport::ShowEquip( )
{
	m_pEquip->Reset();
	return m_pEquip;
}

void TeamFortressViewport::CreateEquip( )
{
	// Create the panel
	m_pEquip = new CEquip(255, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pEquip->setParent(this);
	m_pEquip->setVisible( false );
}
// - Spin
//======================================================================================
...
void TeamFortressViewport::UpdateCursorState()
{
	...
	if( /*gHUD.m_ShowMenuPE.m_iActive || gHUD.m_PETeam.m_iActive ||*/ showmouse )
	{
		//App::getInstance()->setMouseCapture(m_pEquip);
		m_pEquip->Open( );
		g_iVisibleMouse = true;
		App::getInstance()->setCursorOveride( App::getInstance()->getScheme()->getCursor(Scheme::SchemeCursor::scu_arrow) );
		return;
	}
	else if ( m_pSpectatorPanel->m_menuVisible || m_pCurrentMenu || m_pTeamMenu->isVisible() || m_pServerBrowser->isVisible() || GetClientVoiceMgr()->IsInSquelchMode() )
	{
		g_iVisibleMouse = true;
		App::getInstance()->setCursorOveride( App::getInstance()->getScheme()->getCursor(Scheme::SchemeCursor::scu_arrow) );
		return;
	...
	else if( m_pEquip && m_pEquip->isVisible( ) )
		m_pEquip->Close( );
...
//	int temp;
	...
	/*if ( down && ( keynum == K_UPARROW || keynum == K_KP_UPARROW ) )
		if( gHUD.m_ShowMenuPE.m_iActive )
		{
			if( gHUD.m_ShowMenuPE.m_iMenu > 0 )
			{
				temp = gHUD.m_ShowMenuPE.m_iMenu;
				if( ( temp == 4 ) && ( gHUD.m_ShowMenuPE.m_sInfo.team != 0 ) && ( gHUD.m_ShowMenuPE.m_sInfo.team != 3 ) )
					temp--;
				gHUD.m_ShowMenuPE.SelectMenuItem( 0 );
				gHUD.m_ShowMenuPE.SelectMenuItem( temp - 1 );
			}
		}
	if ( down && ( keynum == K_DOWNARROW || keynum == K_KP_DOWNARROW ) )
		if( gHUD.m_ShowMenuPE.m_iActive )
			if( gHUD.m_ShowMenuPE.m_iMenu < 5 )
			{
				temp = gHUD.m_ShowMenuPE.m_iMenu;
				if( ( temp == 2 ) && ( gHUD.m_ShowMenuPE.m_sInfo.team != 0 ) && ( gHUD.m_ShowMenuPE.m_sInfo.team != 3 ) )
					temp++;
				gHUD.m_ShowMenuPE.SelectMenuItem( 0 );
				gHUD.m_ShowMenuPE.SelectMenuItem( temp + 1 );
			}
			else if( gHUD.m_ShowMenuPE.m_iMenu == 5 )
				gHUD.m_ShowMenuPE.SelectMenuItem( 0 );
	if ( down && ( keynum == K_LEFTARROW || keynum == K_KP_LEFTARROW ) )
		if( gHUD.m_ShowMenuPE.m_iActive )
		{
			if( gHUD.m_ShowMenuPE.m_iMenu > 0 )
			gHUD.m_ShowMenuPE.SelectMenuItem( 8 );
		}
	if ( down && ( keynum == K_RIGHTARROW || keynum == K_KP_RIGHTARROW ) )
		if( gHUD.m_ShowMenuPE.m_iActive )
		{
			if( gHUD.m_ShowMenuPE.m_iMenu > 0 )
...
int TeamFortressViewport::MsgFunc_ScoreInfo( const char *pszName, int iSize, void *pbuf )
{
	...
	if ( cl > 0 && cl <= MAX_PLAYERS )
	{
		...
		g_PlayerExtraInfo[cl].level = level;
...
int TeamFortressViewport::MsgFunc_Class( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
	int cl = READ_BYTE( );
	int playerclass = READ_BYTE( );
...
int TeamFortressViewport::MsgFunc_TeamScore( const char *pszName, int iSize, void *pbuf )
{
	...
	g_TeamInfo[1].points = READ_SHORT( );
	g_TeamInfo[2].points = READ_SHORT( );
	//for( int i = 0; i < 5; i++ )
	//	gHUD.m_Counter.m_sCounters[i].iActive = 0;
...
int TeamFortressViewport::MsgFunc_TeamInfo( const char *pszName, int iSize, void *pbuf )
{
	...
	{  
		...
		g_PlayerExtraInfo[cl].teamnumber = READ_BYTE( );
		/*if( gEngfuncs.pEventAPI->EV_IsLocal( cl ) )
			g_iTeamNumber = g_PlayerExtraInfo[cl].teamnumber;*/
	}
...
int TeamFortressViewport::MsgFunc_TargHlth( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );

	g_PlayerInfoList[0].health = READ_BYTE();
	g_PlayerInfoList[0].armor = READ_BYTE();
	UpdateSpectatorPanel( );
	return 1;
}

int TeamFortressViewport::MsgFunc_RecoilRatio( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );

	g_fRecoilRatio = READ_COORD( );
	return 1;
}

int TeamFortressViewport::MsgFunc_Special( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
...
/*int TeamFortressViewport::MsgFunc_Fog( const char *pszName, int iSize, void *pbuf )
{
	...
	g_fFogCol[0] = READ_BYTE( );
	g_fFogCol[1] = READ_BYTE( );
	g_fFogCol[2] = READ_BYTE( );

	g_fFogStart = READ_SHORT( );
	g_fFogEnd = READ_SHORT( );

	g_bFogOn = ( READ_BYTE( ) ? true : false );
...
int TeamFortressViewport::MsgFunc_Rain( const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
	
	iRainIndex = READ_SHORT( );
	return 1;
}*/
