...
int __MsgFunc_Class(const char *pszName, int iSize, void *pbuf)
{
	if (gViewPort)
		return gViewPort->MsgFunc_Class( pszName, iSize, pbuf );
	return 0;
}
...
int __MsgFunc_TargHlth(const char *pszName, int iSize, void *pbuf)
{
	if (gViewPort)
		return gViewPort->MsgFunc_TargHlth( pszName, iSize, pbuf );
	return 0;
}

int __MsgFunc_RecoilRatio(const char *pszName, int iSize, void *pbuf)
{
	if (gViewPort)
		return gViewPort->MsgFunc_RecoilRatio( pszName, iSize, pbuf );
	return 0;
}

int __MsgFunc_Special(const char *pszName, int iSize, void *pbuf)
{
	if (gViewPort)
		return gViewPort->MsgFunc_Special( pszName, iSize, pbuf );
	return 0;
}
...
{
	if (gViewPort)
		return gViewPort->MsgFunc_Fog( pszName, iSize, pbuf );
	return 0;
}*/
...
{
	if (gViewPort)
		return gViewPort->MsgFunc_Rain( pszName, iSize, pbuf );
	return 0;
}*/
...
{
	if( !g_helper )
		g_helper = new cPEHelper( );
	/*if( !strlen(path) )
	{
		char *slash;
		sprintf( path, "%s", gEngfuncs.pfnGetGameDirectory( ) );
		while( slash = strstr( path, "/" ) )
			*slash = '\\';
...
//	m_hsprFont = LoadSprite("sprites/%d_font.spr");
	...
	if ( !m_pSpriteList )
	{
		...
		if (m_pSpriteList)
		{
			...
			m_rghSprites = new cldll::HSPRITE[m_iSpriteCount];
	...
	m_Spectator.VidInit();
	m_Counter.VidInit( );
//	m_ShowMenuPE.VidInit( );
//	m_Intro.VidInit( );
//	m_Briefing.VidInit( );
	m_Lens.VidInit( );
	m_Music.VidInit( );
	m_Radar.VidInit( ); // by ste0
//	m_NVG.VidInit();
	m_Notify.VidInit( );
//	m_PETeam.VidInit( );
	m_PEPointMgr.VidInit( );
}
...
int CHud::MsgFunc_SetFOV(const char *pszName,  int iSize, void *pbuf)
{
	...
	int def_fov = 90;//CVAR_GET_FLOAT( "default_fov" );
