...
cldll::HSPRITE ghsprBuckets;					// Sprite for top row of weapons menu
...
int CHudAmmo::VidInit(void)
{
	...
	g_sprHUD1 = LoadSprite( "sprites/hud/hud1.spr" );
...
void CHudAmmo::Think(void)
{
	...
	if ( gHUD.m_iWeaponBits != gWR.iOldWeaponBits )
	{
        ...
        {
           if( p->iId == 30 ) // OBJECTIVE_ITEM
            g_IsSpecial[gEngfuncs.GetLocalPlayer( )->index] = FALSE;
...
cldll::HSPRITE* WeaponsResource :: GetAmmoPicFromWeapon( int iAmmoId, wrect_t& rect )
{
	for ( int i = 0; i < MAX_WEAPONS; i++ )
	{
...
/*	if ( gHUD.m_ShowMenuPE.m_iActive )	
	{
		gHUD.m_ShowMenuPE.SelectMenuItem( iSlot + 1 );
		return;
	}*/
...
/*	if ( gHUD.m_PETeam.m_iActive )	
	{
		gHUD.m_PETeam.SelectMenuItem( iSlot + 1 );
		return;
	}*/
...
int CHudAmmo::MsgFunc_WeapPickup( const char *pszName, int iSize, void *pbuf )
{
  ...
  if( iIndex == 30 ) // OBJECTIVE_ITEM
    g_IsSpecial[gEngfuncs.GetLocalPlayer( )->index] = TRUE;
...
int CHudAmmo::Draw(float flTime)
{
	...
	else if( m_pWeapon->iAmmoType > 0 )
	{
		...
		if (pw->iClip >= 0)
		{
			...
			ScaleColors(r, g, b, a );
			x = gHUD.DrawHudNumber(x, y, iFlags | DHN_3DIGITS, gWR.CountAmmo(pw->iAmmoType), 255, 255, 255);//r,g,b
	...
	if( m_pWeapon->iAmmo2Type > 0 )
	{
		...
		if (pw->iClip2 >= 0)
		{
			...
			// GL Seems to need this
			ScaleColors(r, g, b, a );
			x = gHUD.DrawHudNumber(x, y, iFlags | DHN_3DIGITS, gWR.CountAmmo(pw->iAmmo2Type), 255, 255, 255);//r,g,b
