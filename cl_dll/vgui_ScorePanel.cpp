...
SBColumnInfo g_ColumnInfo[NUM_COLUMNS] =
{
	...
	{"#CLASS",		40 /*56*/,	Label::a_east},		// class
	{"Level",		  40,			Label::a_east},
	{"#SCORE",		40,			Label::a_east},
	{"#DEATHS",		46,			Label::a_east},
	{"#LATENCY",	36,			Label::a_east},
...
void ScorePanel::SortTeams()
{
	...
	for ( i = 1; i <= m_iNumTeams; i++ )
	{
		...
		g_TeamInfo[i].level = g_TeamInfo[i].ping = g_TeamInfo[i].packetloss = 0;
	}
  ...
  // recalc the team scores, then draw them
	for ( i = 1; i < MAX_PLAYERS; i++ )
	{
		...
		if ( ( g_PlayerExtraInfo[i].teamnumber == 0 ) || ( g_PlayerExtraInfo[i].teamnumber > 2 ) )
		...
		{*/
			...
			g_TeamInfo[j].level += g_PlayerExtraInfo[i].level;
    /*}*/
	...
	//g_TeamInfo[1].frags = g_TeamInfo[1].points;
	//g_TeamInfo[2].frags = g_TeamInfo[2].points;
	...
	for ( i = 1; i <= 2; i++ )
	{
		...
		if ( g_TeamInfo[i].players > 0 )
		{
			...
			g_TeamInfo[i].level /= g_TeamInfo[i].players;  // use the average ping of all the players in the team as the teams ping
		}
	}
	...
	while ( 1 )
	{
		...
		SortPlayers( 0, g_TeamInfo[best_team].teamnumber );
	}
...
//-----------------------------------------------------------------------------
void ScorePanel::SortPlayers( int iTeam, int team )
{
	...
	while ( 1 )
	{
		...
		for ( int i = 1; i < MAX_PLAYERS; i++ )
		{
			if ( m_bHasBeenSorted[i] == false && g_PlayerInfoList[i].name && g_PlayerExtraInfo[i].frags >= highest_frags )
			{
				...
				if ( ent && (/*team && */( g_PlayerExtraInfo[i].teamnumber == team ) ) )  
				{
		...
		for(int col=0; col < NUM_COLUMNS; col++)
		{
			...
			if ( m_iIsATeam[row] )
			{
				...
				case COLUMN_NAME:
					...
					else
					{
						...
						sprintf( sz2, g_TeamInfo[team_info->teamnumber].name );
					}
			...
			else
			{
				...
				case COLUMN_CLASS:
					...
					//if ( gViewPort && ( g_iTeamNumber == g_PlayerExtraInfo[ m_iSortedRows[row] ].teamnumber ) )//EV_TFC_IsAllyTeam( g_iTeamNumber, g_PlayerExtraInfo[ m_iSortedRows[row] ].teamnumber )  )
					//	bShowClass = true;
					...
					////	bool bNoClass = false;
						...
						//	sprintf(sz, "");
						//else
					//if( ( g_iTeamNumber > 0 ) && ( g_iTeamNumber < 3 ) )
