...
=================
*/
void EV_GetDefaultShellInfo( event_args_t *args, float *origin, float *velocity, float *ShellVelocity, float *ShellOrigin, float *forward, float *right, float *up, float forwardScale, float upScale, float rightScale, int xh, int yh, int zh )
{
	int i;
	vec3_t view_ofs;
	float fR, fU;
	...
	for ( i = 0; i < 3; i++ )
	{
		...
		ShellOrigin[i] = ShellOrigin[i] + ( 2 + xh ) * forward[i] + ( 5 + yh ) * right[i] + ( 6 + zh ) * up[i];
	}
}
