...
kbutton_t	in_mouse;  // Display the netgraph
...
int KB_ConvertString( char *in, char **ppout )
{
	...
	*ppout = NULL;
	...
	while ( *p )
	{
		if ( *p == '+' )
		{
			char *pBindEnd = binding;
			while ( *p && ( isalnum( *p ) || ( pBindEnd == binding ) ) && ( ( pBindEnd - binding ) < 63 ) )
			{
				*pBindEnd++ = *p++;
			}
...
//	RecClCL_CreateMove(frametime, cmd, active);
	...
	if( /*gHUD.m_ShowMenuPE.m_iActive || */GetClientVoiceMgr()->IsInSquelchMode())
...
void InitInput (void)
{
	...
	gEngfuncs.pfnAddCommand ("+mouse",IN_MouseDown);
	gEngfuncs.pfnAddCommand ("-mouse",IN_MouseUp);
  ...
  cl_forwardspeed = new cvar_t;
  cl_backspeed = new cvar_t;
  cl_sidespeed = new cvar_t;
  cl_forwardspeed->value = 300;
  cl_backspeed->value = 300;
  cl_sidespeed->value = 300;
	cl_movespeedkey		= gEngfuncs.pfnRegisterVariable ( "cl_movespeedkey", "0.45", 0 );
