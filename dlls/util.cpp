...
{
	MESSAGE_BEGIN( MSG_PVS, gmsgSpray, origin );
...
void UTIL_Remove( CBaseEntity *pEntity )
{
  ...
  if( FClassnameIs( pEntity->pev, "monster_zombie" ) && !pEntity->pev->fuser4 )
  {
    pEntity->SetThink( NULL );
    SetBits( pEntity->pev->flags, FL_DORMANT );
    ClearBits( pEntity->pev->flags, FL_MONSTER );
    zombiepool.push_back( pEntity );
  ...
  else
  {
    ...
    pEntity->SetThink(&CBaseEntity::SUB_Remove);//REMOVE_ENTITY( pEntity->edict( ) );
  }
}
...
int CSave :: WriteFields( const char *pname, void *pBaseData, TYPEDESCRIPTION *pFields, int fieldCount )
{
	...
	for ( i = 0; i < fieldCount; i++ )
	{
		...
		case FIELD_FUNCTION:
			WriteFunction( pTest->fieldName, (void **)(char *)pOutputData, pTest->fieldSize );
		break;
		default:
...
// + ste0
BOOL UTIL_FromChance ( float chance )
{
	float rnd;
	rnd = RANDOM_FLOAT ( 0 , 100);
