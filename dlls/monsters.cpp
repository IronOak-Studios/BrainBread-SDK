...
// perhaps MaintainActivity() or a ShiftAnimationOverTime() or something.
    ...
    {
		  if ( m_fSequenceLoops )
        iSequence = ( RANDOM_LONG( 1, 4 ) < 4 ? 2 : 1 );//LookupActivity ( m_Activity );
		  else
			  iSequence = 0;//LookupActivityHeaviest ( m_Activity );
    }
    else if( FClassnameIs( pev, "monster_hgrunt" ) || FClassnameIs( pev, "monster_hgrunt_shotgun" ) )
    {
     if ( m_fSequenceLoops )
        iSequence = 12;//LookupActivity ( m_Activity );
		  else
			  iSequence = 11;//LookupActivityHeaviest ( m_Activity );
    }
    //else
    /*{
      if ( m_fSequenceLoops )
...
{
	// Don't set state to ALERT here because it can resurrect dying monsters
...
//		UTIL_ParticleEffect ( pev->origin, g_vecZero, 255, 25 );
		...
		if ( !WALK_MOVE( ENT(pev), flYaw, stepSize * 2, WALKMOVE_CHECKONLY ) )
		{// can't take the next step, fail!
			...
			else
			{
				...
				if ( pTarget && fabs( flDist - flStep ) < LOCAL_STEP_SIZE )
					iReturn = LOCALMOVE_VALID;
				else
...
//							ALERT( at_aiconsole, "Wating for door %.2f\n", m_flMoveWaitFinished-gpGlobals->time );
		...
		{
			if ( distance < m_flGroundSpeed * 0.6 /* FIX */ )
			{
				MovementComplete();
			}
		}
...
//	WALK_MOVE( ENT(pev), flYaw, m_flGroundSpeed * flInterval, WALKMOVE_NORMAL );
  ...
  {
    vec3_t targ = tr.vecEndPos + dir * 2;
    vec3_t targ1 = tr.vecEndPos + dir * MAX_LEN;

    // check height
    UTIL_TraceLine( Vector( targ.x, targ.y, targ.z + MAX_HEIGHT ), targ, ignore_monsters, dont_ignore_glass, ENT( pev ), &tr1 );
    // check dist
    UTIL_TraceLine( targ1, targ + dir * 2, ignore_monsters, dont_ignore_glass, ENT( pev ), &tr2 );
    if( tr1.flFraction < 1.0 && tr1.pHit->v.solid == SOLID_BSP && tr1.pHit == tr2.pHit )
      step = true; // Can walk over it
  }
  /*//if( !step )
  //{
    //ALERT( at_console, "not stepping\n" );
    vec3_t start = pev->origin + dir * 20;
    UTIL_TraceLine( start, start - Vector( 0, 0, 250 ), ignore_monsters, dont_ignore_glass, ENT( pev ), &tr1 );
    if( tr1.flFraction < 1.0 && tr1.pHit->v.solid == SOLID_BSP && ( tr1.vecEndPos - start ).Length( ) >= 40 )
    {
      ALERT( at_console, "can go down\n" );
      ALERT( at_console, "%f %f %f -> %f %f %f\n", start.x, start.y, start.z, tr1.vecEndPos.x, tr1.vecEndPos.y, tr1.vecEndPos.z );
      m_Route[ m_iRouteIndex ].vecLocation = tr1.vecEndPos + dir * 30;
      UTIL_SetOrigin( pev, tr1.vecEndPos + dir * 30 );
      this->RouteNew( );
      return;
    }
    //else
  ...
  while (flTotal > 0.001)
    ...
    if( step && Distance( tr.vecEndPos, pev->origin ) <= 20 )
    {
        ...
        UTIL_SetOrigin( pev, m_Route[ m_iRouteIndex ].vecLocation );
//		ALERT( at_console, "Stepped over obstacle\n" );
		step = false;
    }
    else
...
int CBaseMonster::IRelationship ( CBaseEntity *pTarget )
{
	...
	{			 //   NONE	 MACH	 PLYR	 ZOMB	 HMIL	 AMIL	 APASS	 AMONST	APREY	 APRED	 INSECT	PLRALY	PBWPN	ABWPN
	/*NONE*/		{ R_NO	,R_NO	,R_NO	,R_NO	,R_NO	,R_NO	,R_NO	,R_NO	,R_NO	,R_NO	,R_NO	,R_NO,	R_NO,	R_NO	},
	/*MACHINE*/		{ R_NO	,R_FR	,R_NO	,R_NM	,R_NO	,R_NO	,R_DL	,R_DL	,R_DL	,R_DL	,R_NO	,R_DL,	R_DL,	R_DL	},
	/*PLAYER*/		{ R_NO	,R_NO	,R_NO	,R_NM	,R_NO	,R_DL	,R_DL	,R_DL	,R_DL	,R_DL	,R_NO	,R_NO,	R_DL,	R_DL	},
	/*ZOMBIES*/     { R_NO	,R_NO	,R_NM	,R_NO	,R_NM	,R_NM	,R_NO	,R_HT	,R_DL	,R_FR	,R_NO	,R_AL,	R_NO,	R_NO	},
	/*HUMANMILITAR*/{ R_NO	,R_NO	,R_HT	,R_NM	,R_FR	,R_NO	,R_DL	,R_DL	,R_DL	,R_DL	,R_NO	,R_HT,	R_NO,	R_NO	},
...
CBaseEntity *CBaseMonster :: BestVisibleEnemy ( void )
{
	...
	while ( pNextEnt != NULL )
	{
    if ( pNextEnt->IsAlive() && pNextEnt->pev->team != pev->team )
		{
