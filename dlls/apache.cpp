...
LINK_ENTITY_TO_CLASS( mapobject_zeppelin, CApache );

void CApache ::ReSpawn ()
{
	pev->effects = 0;
	...
	SET_MODEL(ENT(pev), "models/mapobjects/zeppelin/zeppelin.mdl");
...
{
	PRECACHE_MODEL("models/mapobjects/zeppelin/zeppelin.mdl");
...
void CApache :: Flight( void )
{
	...
	if (m_iSoundState == 0)
	{
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "apache/ap_rotor2.wav", 0.5, 0.3, 0, 110 );
	...
	else
	{
		...
		if (pPlayer)
		{
			...
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "apache/ap_rotor2.wav", 0.5, 0.3, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
		}
