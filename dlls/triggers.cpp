...
extern int gmsgNVGActive; // by steo
...
extern entvars_t *GetObjectiveSpawn( int flagSet );

// When touched, a hurt trigger does DMG points of damage each half-second
void CBaseTrigger :: HurtTouch ( CBaseEntity *pOther )
{
	float fldmg;
  if( pOther && !strcmp( STRING( pOther->pev->classname ), "weaponbox" ) )
	{
		if( pOther->pev->fuser4 )
		{
      DisableAll( "weaponbox" );
		  ((CWeaponBox*)pOther)->Kill( );
      ...
      if( !FNullEnt( pent ) && spwn )
      {
        ...
        ((cPEHacking*)g_pGameRules)->misDone[MISSION_OBJECT] = 1;

			NotifyAll( NTM_OBJECTRESTORE );
		}
    else
      UTIL_Remove( pOther );
  }
	/*if( pOther && !strcmp( STRING(pOther->pev->classname), "weaponbox" ) )
	{
		if( g_iPERules == 1 )
		{
			CBaseEntity *cb = UTIL_FindEntityByClassname( NULL, "pe_spawn_syndicate" );
			if( cb == NULL )
				cb = UTIL_FindEntityByClassname( cb, "pe_spawn_syndicate" );
			if( cb )
			{
				UTIL_SetOrigin( pOther->pev, cb->pev->origin );
				NotifyAll( NTM_HTOOLRESTORE );
			}
		}
		else
		{
			CBaseEntity *cb = UTIL_FindEntityByClassname( NULL, "pe_object_case" );
			if( cb == NULL )
				cb = UTIL_FindEntityByClassname( cb, "pe_object_case" );
			if( cb )
			{
				UTIL_SetOrigin( pOther->pev, cb->pev->origin );
				NotifyAll( NTM_CASERESTORE );
			}
		}
		return;
	}*/
...
void CTriggerCamera::FollowTarget( )
{
	...
	if( ( m_hPlayer->pev->velocity.y > 30 ) ||( m_hPlayer->pev->velocity.x > 30 ) || ( m_hTarget == NULL ) || ( m_flReturnTime <= gpGlobals->time ) )
	{
