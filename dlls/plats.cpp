...
{
	// Platform was removed, remove orphaned trigger
	...
	CFuncPlat *platform = (CFuncPlat *)(CBaseEntity *)m_hPlatform;
...
void CFuncTrain::ReSpawn( )
{
	...
	m_pevCurrentTarget = m_pStartTarget;
	pev->target = m_pTarget;
	pev->velocity = g_vecZero;
	pev->nextthink = 0;
	
	SetBits(pev->effects, EF_NOINTERP);
	UTIL_SetOrigin(pev, m_pOrigin );
	Next( );
//	Wait( );
//	m_pevCurrentTarget = m_pStartTarget;
//	pev->target = m_pTarget;
...
void CFuncTrain :: Spawn( void )
{
	...
	m_pStartTarget = m_pevCurrentTarget;
	m_pOrigin = pev->origin;
	m_pTarget = pev->target;
