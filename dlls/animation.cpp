...
int BuildHashList( void **first, void *pmodel )
{
	studiohdr_t *psh;
	s_hashitem *hlist;
	
	psh = (studiohdr_t *)pmodel;
	if( !psh )
		return 0;

	mstudioseqdesc_t	*psd;

	psd = (mstudioseqdesc_t *)((byte *)psh + psh->seqindex);

	hlist = new s_hashitem[psh->numseq];

	for( int i = 0; i < psh->numseq; i++ )
	{
		hlist[i].key = ElfHash( psd[i].label );
		hlist[i].value = i;
	}
...
int GetAnimationEvent( void *pmodel, entvars_t *pev, MonsterEvent_t *pMonsterEvent, float flStart, float flEnd, int index )
{
	...
	pstudiohdr = (studiohdr_t *)pmodel;
	if ( !pstudiohdr || pev->sequence >= pstudiohdr->numseq || !pMonsterEvent || pev->sequence < 0)
		return 0;
