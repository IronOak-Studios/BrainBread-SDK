...
void ClientPutInServer( edict_t *pEntity )
{
	...
	// Spin
	((cPEHacking*)g_pGameRules)->m_iClients++;
	((cPEHacking*)g_pGameRules)->m_iPlayers[0]++;
	
	pPlayer->m_iHUDInited = FALSE;
	pPlayer->m_fPointsMax = 0;
	pPlayer->m_iClientPointsMax = (int)pPlayer->m_fPointsMax;
	pPlayer->Spawn( );
...
// turn on color set 2  (color on,  no sound)
	...
	{
		if ( !client || !client->IsPlayer( ) || !client->pev )
			continue;
...
void ClientCommand( edict_t *pEntity )
{
	...
	{ 
		...
		if (pPlayer->m_fNVG)
		{
			if (pPlayer->m_fNVGActive)
				pPlayer->NVGActive(FALSE);
			else
				pPlayer->NVGActive(TRUE);
		}
		else ALERT (at_console, "You do not have a nvg\n"); // ste0
	}
    ...
    else if ( FStrEq(pcmd, "follownext" ) )
    {
		...
		{
		case 1:
			GetClassPtr((CBasePlayer *)pev)->Observer_FindNextPlayer( 1 );
			break;
		case 0:
			GetClassPtr((CBasePlayer *)pev)->Observer_FindNextPlayer( 0 );
			break;
		}
    }
    /*else if ( FStrEq(pcmd, "live" ) )
    {
		CBasePlayer *pPlayer = GetClassPtr((CBasePlayer *)pev);
	...
	else if ( FStrEq(pcmd, "drop" ) )
	{
		...
		if( !pPlayer->m_pActiveItem || FClassnameIs( pPlayer->m_pActiveItem->pev, "weapon_knife" )  )
			return;
		if( !pPlayer->m_pActiveItem || FClassnameIs( pPlayer->m_pActiveItem->pev, "weapon_hand" )  )
			return;
		if( FClassnameIs( pPlayer->m_pActiveItem->pev, "weapon_canister" ) )
    {
    ...
    sprintf( text, "%s", STRING(pPlayer->m_pActiveItem->pev->classname) );
	  ...
	  if( pPlayer->m_sLastWeap )
			...
			GetClassPtr((CBasePlayer *)pev)->SelectItem( "weapon_knife" );
		return;
	}
...
void ClientUserInfoChanged( edict_t *pEntity, char *infobuffer )
{
	...
	if ( pEntity->v.netname && STRING(pEntity->v.netname)[0] != 0 && !FStrEq( STRING(pEntity->v.netname), g_engfuncs.pfnInfoKeyValue( infobuffer, "name" )) )
	{
		...
		UTIL_LogPrintf( "\"%s<%i><%s><%s>\" changed name to \"%s\"\n", 
			...
			GETPLAYERUSERID( pEntity ), 
			GETPLAYERAUTHID( pEntity ),
			g_engfuncs.pfnInfoKeyValue( infobuffer, "team" ), 
...
// player pain sounds
	...
	//PE
	PRECACHE_SOUND("radio/rnddraw.wav");

	PRECACHE_SOUND("radio/radio1/attack.wav");
	PRECACHE_SOUND("radio/radio1/coverme.wav");
	PRECACHE_SOUND("radio/radio1/advance.wav");
	PRECACHE_SOUND("radio/radio1/staytoge.wav");
	PRECACHE_SOUND("radio/radio1/complete.wav");
	PRECACHE_SOUND("radio/radio1/retreat.wav");
	PRECACHE_SOUND("radio/radio1/incoming.wav");
	PRECACHE_SOUND("radio/radio1/waitorde.wav");

	PRECACHE_SOUND("radio/radio2/ceasef.wav");
	PRECACHE_SOUND("radio/radio2/openfire.wav");
	PRECACHE_SOUND("radio/radio2/im_hit.wav");
	PRECACHE_SOUND("radio/radio2/needassi.wav");
	PRECACHE_SOUND("radio/radio2/holdpos.wav");
	PRECACHE_SOUND("radio/radio2/follow.wav");
	PRECACHE_SOUND("radio/radio2/outofamo.wav");
	PRECACHE_SOUND("radio/radio2/in_pos.wav");

	PRECACHE_SOUND("radio/radio3/attentio.wav");
	PRECACHE_SOUND("radio/radio3/ambush.wav");
	PRECACHE_SOUND("radio/radio3/granade.wav");
	PRECACHE_SOUND("radio/radio3/sniper.wav");
	PRECACHE_SOUND("radio/radio3/roger.wav");
	PRECACHE_SOUND("radio/radio3/negative.wav");
	PRECACHE_SOUND("radio/radio3/matedown.wav");
}
...
int AddToFullPack( struct entity_state_s *state, int e, edict_t *ent, edict_t *host, int hostflags, int player, unsigned char *pSet )
{
	...
	if ( ent != host )
	{
		...
		{
			//if( player )
			//	SetBits( ent->v.flags, FL_IMMUNE_LAVA );
			return 0;
		}
		//ClearBits( ent->v.flags, FL_IMMUNE_LAVA );
	}
			...
			while ( pPlayerItem )
			{
				...
				if ( gun && gun->UseDecrement() )
				{
					...
					if ( II.iId >= 0 && II.iId < 32 )
					{
						...
						item->m_flTimeWeaponIdle		= max( (float)gun->m_flTimeWeaponIdle, -0.001f );
						item->m_flNextPrimaryAttack		= max( (float)gun->m_flNextPrimaryAttack, -0.001f );
						item->m_flNextSecondaryAttack	= max( (float)gun->m_flNextSecondaryAttack, -0.001f );
						item->m_fInReload				= gun->m_fInReload;
						item->m_fInSpecialReload		= gun->m_fInSpecialReload;
						item->fuser1					= max( (float)gun->pev->fuser1, -0.001f );
...
void UpdateClientData ( const struct edict_s *ent, int sendweapons, struct clientdata_s *cd )
{
	...
	if ( sendweapons )
	{
		...
		if ( pl )
		{
			...
			cd->vuser1.y		= pl->ammo_slot5;
			cd->vuser1.z		= pl->ammo_slot4;
			cd->ammo_nails		= pl->ammo_slot2_a;
			cd->ammo_shells		= pl->ammo_slot3_a;
			cd->ammo_rockets	= pl->ammo_slot4_a;
			cd->ammo_cells		= pl->ammo_slot2;
			cd->vuser2.x		= pl->ammo_slot3;
