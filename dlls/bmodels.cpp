...
class CPEClip : public CFuncWall
{
	CBaseEntity *m_pLinkedEnt;
public:
	void	Spawn( );
	void	ReSpawn( );
...
LINK_ENTITY_TO_CLASS( pe_objectclip, CPEClip );

int CPEClip::TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType )
{
	if( m_pLinkedEnt && ( m_pLinkedEnt != this ) )
	{
		m_pLinkedEnt->TakeDamage( pevInflictor, pevAttacker, flDamage, bitsDamageType );
		//ALERT( at_console, "%s, %s will now take damage\n", STRING(m_pLinkedEnt->pev->classname), STRING(m_pLinkedEnt->pev->targetname) );
		return 1;
	}
	return 0;
}

void CPEClip::Spawn( void )
{
	CFuncWall::Spawn( );
	/*if ( CVAR_GET_FLOAT("showtriggers") == 0 )
		pev->effects = EF_NODRAW;*/
...
void CPEClip::ReSpawn( void )
{
	m_pLinkedEnt = NULL;

	CBaseEntity *linked = UTIL_FindEntityByTargetname( NULL, STRING(pev->targetname) );
	if( linked != NULL )
	{
		m_pLinkedEnt = linked;			
		//ALERT( at_console, "%s has name %s and is kewl\n", STRING(linked->pev->classname), STRING(linked->pev->targetname) );
	}

	if( m_pLinkedEnt == this )
	{
		linked = UTIL_FindEntityByTargetname( m_pLinkedEnt, STRING(pev->targetname) );
		if( linked != NULL )
		{
			m_pLinkedEnt = linked;			
			//ALERT( at_console, "%s has name %s and is kewl\n", STRING(linked->pev->classname), STRING(linked->pev->targetname) );
